...
- [ ] Create `marketplace/mcp.json` containing curated starter remote MCP servers:
  - `todoist`: Official hosted Todoist MCP server (`https://ai.todoist.net/mcp`) supporting OAuth 2.1 & Personal Developer API Token.
- [ ] Ensure JSON schema is extensible for future additions (Web Search, GitHub, etc.).
...
- [ ] Register Obsidian protocol handler in `src/main.ts`:
...
- [ ] Implement `src/mcp/mcp-manager.ts`:
...
- [ ] Implement `src/tools/mcp/bridge-tool.ts`:
...
- [ ] Implement `src/ui/components/mcp-tools-view-modal.ts`:
  - Modal inspecting tool schemas, argument properties, and types.
- [ ] Implement `src/ui/components/mcp-server-edit-modal.ts`:
